...
$(function() {
    ...
    $(document).on('click', '.btn-item-show', function(evt) {     
        ...
        console.log(Codigo);
        let articuloId = $(this).closest('tr').data('articuloId')
        console.log(articuloId);
        fn_CargarModalArticulosVenta(function() {
    ...
    $('#btn-add-pago').on('click', function () {
        ...
        let usuarioId = window.usuarioSesion.usuario_id;
        let usuario = (window.usuarioSesion.nombre + ' ' + window.usuarioSesion.apellido_paterno).toUpperCase();        
		...
		tr.append($('<td>').text(fechaM.format("DD/MM/YYYY HH:mm")));
		tr.append($('<td>').text(usuario).data('usuarioId', usuarioId));
		tr.append($('<td>').html("<button class='btn btn-sm btn-danger btn-remove-pago'><i class='fa fa-trash'></i></button>"));
...
function fn_CargarModalArticulosVenta(success, showModal = true) {
