$(document).ready(function () {
    ...
    // if (usuario.rol_id != 1) { $('#tienda').prop('disabled', true); }
    ...
    */
    ...
    $('#btn-sale-register-modal-show').click(function(evt) {
        evt.preventDefault();
        fn_ObtenerModalRegistrarVenta(function() {
            // console.log('mostrar modal');
            $('#modal-sale-add').modal({ show: true });
        });        
    })
})
...
var fn_ListarVentas = function () {
    table = $('#tbl_sales').DataTable({
        ...
        columns: [
            ...
            // },
            {
                data: "ventaId", render: function (data, type, row) {
                    ...
                    .addClass('btn btn-link btn-sale-show')
                    .attr('data-venta', data)
                    .append((row.numeroOperacion=="") ? zfill(data, 6) : row.numeroOperacion)).html();
                    // .append(zfill(data, 6))).html();
                }
            },
            // {
            ...
            { data: "fechaSalidaProgramada", render: function (data) { 
                if (data != null){
                    var fecha = reviver('', data); return fecha.format()  
                } else {
                    return "";
                }    
            }                
        ...
        createdRow: function (row, data, dataIndex, cells) {            
            ...
            let menuDisabled = [];
            if (usuario.rol_id != 1) {
                menuDisabled.push('mnAnular');
            }

            $(row).find('td').contextMenu({
                menuSelector: "#contextMenuVentas",
...
function fn_MostrarMenu (invokedOn, selectedMenu) {
    let TipoDoc, SerieDoc, NumDoc;
    let ventaId;
    let clienteId, nroDocumento;
    switch (selectedMenu.attr('id')) {
        case "mnIrAOperacion":            
            ventaId = $(invokedOn).closest('tr').find('td').eq(0).find('a').data('venta');  
            Call_Progress(true)
            setTimeout(function () {
                Call_Progress(true);
                window.open('/ventas/editar/' + ventaId, '_blank');
        ...
        .then((willDelete) => {
            ...
            if (willDelete) {
                ...
                return $.ajax({
                    ...
                    data: { ventaId: ventaId, motivo: willDelete }
                });                
            }
        })
        .then((result) => {
            ...
            if (result) {
                ...
                swal('Por favor, ingrese un motivo de anulación');
            }
        });
}
